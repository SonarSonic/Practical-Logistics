...
public class GuiItemRouter extends GuiSonar {
	...
	public void initGui() {
		...
		if (state == MAIN) {
			...
			this.buttonList.add(new SideButton(0, guiLeft + 9, guiTop + 20, 47, 12, EnumFacing.getFront(handler.side.getObject()).name()));
			for (int i = 1; i < 8; i++) {
	...
	public void drawGuiContainerForegroundLayer(int x, int y) {
		if (state == MAIN) {
			FontHelper.textCentre(FontHelper.translate("tile.ItemRouter.name"), xSize, 7, 1);
			GL11.glTranslated(0, 0.5, 0);
			FontHelper.textOffsetCentre(EnumFacing.getFront(handler.side.getObject()).name(), 31, 22, -1);
			GL11.glTranslated(0, -0.5, 0);
					...
					if (i < filters.size() && Y < finish) {
						...
						if (filter instanceof ItemStackFilter) {
							...
							if (stackFilter.getFilters().get(0) != null) {
								...
								list.add(1, (TextFormatting.GRAY) + "Use NBT: " + (stackFilter.matchNBT ? TextFormatting.WHITE : "") + stackFilter.matchNBT);
								list.add(2, (TextFormatting.GRAY) + "Use OreDict: " + (stackFilter.matchOreDict ? TextFormatting.WHITE : "") + stackFilter.matchOreDict);
								list.add(3, (TextFormatting.GRAY) + "Ignore Damage: " + (stackFilter.ignoreDamage ? TextFormatting.WHITE : "") + stackFilter.ignoreDamage);
								list.add(4, (TextFormatting.GRAY) + "Use Modid: " + (stackFilter.matchModid ? TextFormatting.WHITE : "") + stackFilter.matchModid);
			...
			FontHelper.textCentre(FontHelper.translate("Create Item Filter"), xSize, 9, 1);
			super.drawGuiContainerForegroundLayer(x, y);
		} else if (state == ORE) {
			FontHelper.textCentre(FontHelper.translate("Create Ore Filter"), xSize, 9, 1);
					...
					if (x - guiLeft >= 13 && x - guiLeft <= 152 && y - guiTop >= 32 && y - guiTop <= 32 + (7 * 18)) {
						...
						if (Y < finish) {
							if (button == 0 && !switching) {
								...
								SonarCore.network.sendToServer(new PacketByteBuf(handler, tile.getPos(), 8));
							} else {
								if (i < filters.size() && filters.get(i) != null) {
									...
									if (filter instanceof ItemStackFilter) {
										handler.editStack = i;
										handler.editOre = -1;
										SonarCore.network.sendToServer(new PacketByteBuf(handler, tile.getPos(), 9));
										switchState(1);
									} else {
										handler.editStack = -1;
										handler.editOre = i;
	...
	protected void mouseReleased(int x, int y, int id) {
		super.mouseReleased(x, y, id);
		...
		if (button != null) {
			if (state == MAIN) {
				if (button.id == 3) {
					...
					SonarCore.network.sendToServer(new PacketByteBuf(handler, tile.getPos(), button.id));
					if (button.id == 0) {
						this.currentScroll = 0;
						reset();
					}
				...
				if (button.id == 4) {
					SonarCore.network.sendToServer(new PacketByteBuf(handler, tile.getPos(), 2));
				}
				if (button.id == 6) {
					switching = true;
					SonarCore.network.sendToServer(new PacketByteBuf(handler, tile.getPos(), -1));
					handler.editStack = -1;
					switchState(0);
				}
			} else if (state == ORE) {
	...
	public void switchState(int state) {
		switching = true;
		this.currentScroll = 0;
		Logistics.network.sendToServer(new PacketRouterGui(tile.getPos(), state));
	...
	public class ListButton extends AnimatedButton {
		public int id;
		...
		public void func_146111_b(int x, int y) {
			...
			switch (id) {
			case 1:
				MachineSideConfig config = handler.sideConfigs[handler.side.getObject()].getObject();
				String type = Character.toUpperCase(config.getName().charAt(0)) + config.getName().substring(1);
		...
		public int getTextureX() {
			if (id == 1) {
				MachineSideConfig config = handler.sideConfigs[handler.side.getObject()].getObject();
				int id = config.isInput() ? 1 : config.isOutput() ? 2 : 0;
				return 176 + (id * 12);
			}
	...
	public class FilterButton extends AnimatedButton {
		...
		public void func_146111_b(int x, int y) {
			...
			switch (id) {
			case 0:
				text = (filter.matchNBT ? TextFormatting.WHITE : TextFormatting.GRAY) + "Use NBT: " + filter.matchNBT;
				break;
			case 1:
				text = (filter.matchOreDict ? TextFormatting.WHITE : TextFormatting.GRAY) + "Use OreDict: " + filter.matchOreDict;
				break;
			case 2:
				text = (filter.ignoreDamage ? TextFormatting.WHITE : TextFormatting.GRAY) + "Ignore Damage: " + filter.ignoreDamage;
				break;

			case 3:
				text = (filter.matchModid ? TextFormatting.WHITE : TextFormatting.GRAY) + "Use MODID: " + filter.matchModid;
				break;
			case 4:
				int listType = handler.listType.getObject();
				text = listType == 0 ? "Whitelist" : "Blacklist";
