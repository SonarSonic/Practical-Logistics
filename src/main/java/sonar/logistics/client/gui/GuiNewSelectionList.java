...
public abstract class GuiNewSelectionList<T extends IMonitorInfo> extends GuiLogistics {

	public GuiNewSelectionList(Container container, IWorldPosition part) {
	...
	public ArrayList<ArrayList<Integer>> getListTypes() {
		...
		if (infoList != null) {
			for (int i = start; i < finish; i++) {
				...
				if (!info.isHeader() && isSelectedInfo(info))
					data.add(i - start);
				if (!info.isHeader() && isPairedInfo(info))
					paired.add(i - start);
			}
		}
