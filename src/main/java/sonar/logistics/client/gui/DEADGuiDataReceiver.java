...
public class DEADGuiDataReceiver extends GuiSonar {
	...
	public DEADGuiDataReceiver(InventoryPlayer inventory, TileEntityDataReceiver tile) {
	...
	public int getDataPosition() {
		if (tile.emitter.getCoords() == null) {
			return -1;
		}
		...
		for (int i = start; i < finish; i++) {
			if (tile.emitters.get(i) != null) {
				IdentifiedCoords info = tile.emitters.get(i);
				if (BlockCoords.equalCoords(info.coords, tile.emitter.getCoords().coords)) {
					return i - start;
	...
	public void drawGuiContainerForegroundLayer(int x, int y) {
		...
		if (tile.emitters != null) {
			...
			for (int i = start; i < finish; i++) {
				IdentifiedCoords emitter = tile.emitters.get(i);
				if (emitter != null) {
				...
				if (tile.emitters != null) {
					...
					if (network < emitterSize()) {
						if (tile.emitters.get(network) != null) {
							Logistics.network.sendToServer(new PacketCoordsSelection(tile.xCoord, tile.yCoord, tile.zCoord, tile.emitters.get(network)));
						}
					}
				}
			}
