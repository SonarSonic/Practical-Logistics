...
public class RotaryCraftProvider extends TileProvider {
	...
	public void getTileInfo(List<ILogicInfo> infoList, World world, int x, int y, int z, ForgeDirection dir) {
		...
		if (handler instanceof TemperatureTile) {
			...
			infoList.add(new LogicInfo(id, 0, 0, tile.getTemperature()).addSuffix("degrees"));
			infoList.add(new LogicInfo(id, 0, 0, tile.getMaxTemperature()).addSuffix("degrees"));
		}
		if (handler instanceof ShaftMachine) {
			ShaftMachine tile = (ShaftMachine) handler;
			infoList.add(new LogicInfo(id, 0, 0, tile.getOmega()).addSuffix("rad/s"));
			infoList.add(new LogicInfo(id, 0, 0, tile.getTorque()).addSuffix("Nm"));
			infoList.add(new LogicInfo(id, 0, 0, tile.getPower()).addSuffix("watts"));
		}
		if (handler instanceof PowerAcceptor) {
			PowerAcceptor tile = (PowerAcceptor) handler;
			infoList.add(new LogicInfo(id, 0, 0, tile.isReceiving()));
		}
		if (handler instanceof PowerGenerator) {
			PowerGenerator tile = (PowerGenerator) handler;
			infoList.add(new LogicInfo(id, 0, 0, tile.getCurrentPower()).addSuffix("watts"));
		}
		if (handler instanceof TensionStorage) {
			infoList.add(new LogicInfo(id, 0, 0, handler.getBlockMetadata()).addSuffix("watts"));
		}
	}
