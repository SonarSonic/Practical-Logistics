...
public class RFEnergyProvider extends TileProvider {
	...
	public void getTileInfo(List<ILogicInfo> infoList, World world, int x, int y, int z, ForgeDirection dir) {
		...
		if (handler instanceof IEnergyConnection) {
			...
			infoList.add(new LogicInfo(id, 0, 0, "" + canConnect));
		}

		if (handler instanceof IEnergyStorage) {

			IEnergyStorage info = (IEnergyStorage) handler;
			infoList.add(new LogicInfo(id, 0, 1, info.getEnergyStored()).addSuffix("RF"));
			infoList.add(new LogicInfo(id, 0, 2, info.getMaxEnergyStored()).addSuffix("RF"));
			...
			if (maxStored != 0) {
				infoList.add(new LogicInfo(id, 0, 3, energyStored).addSuffix("RF"));
				infoList.add(new LogicInfo(id, 0, 4, maxStored).addSuffix("RF"));
			}
		}
