...
public class FluidInventoryInteraction extends InfoInteractionHandler<FluidInventoryInfo> {
	...
	@Override
	public void handleInteraction(FluidInventoryInfo info, ScreenType type, TileEntity screen, EntityPlayer player, int x, int y, int z, BlockInteraction interact, boolean doubleClick) {
				...
				if (slot >= 0 && slot < info.stacks.size()) {
					...
					if (stack != null) {
						if (interact.type == BlockInteractionType.LEFT) {
							LogisticsAPI.getFluidHelper().fillHeldItem(player, cache, stack.setStackSize(1000));
						} else if (interact.type == BlockInteractionType.SHIFT_LEFT) {
							LogisticsAPI.getFluidHelper().fillHeldItem(player, cache, stack);
						}
					}
				}
			}
