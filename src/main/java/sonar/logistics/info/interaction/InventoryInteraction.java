...
public class InventoryInteraction extends InfoInteractionHandler<InventoryInfo> {
	...
	@Override
	public void handleInteraction(final InventoryInfo info, ScreenType type, TileEntity screen, EntityPlayer player, int x, int y, int z, BlockInteraction interact) {
		...
		if (interact.type == BlockInteractionType.RIGHT || interact.type == BlockInteractionType.SHIFT_RIGHT) {
			if (interact.type == BlockInteractionType.RIGHT) {
				if (player.getHeldItem() != null) {
					Logistics.network.sendToServer(new PacketScreenInteraction.PacketItemStack(screen.xCoord,screen.yCoord,screen.zCoord, x, y, z, interact, new StoredItemStack(player.getHeldItem())));
					/*
					if (!doubleClick) {
			...
			if (sizing != null) {
				...
				if (slot >= 0 && slot < info.stacks.items.size()) {
					StoredItemStack stack = info.stacks.items.get(slot);
					if (stack != null) {
						Logistics.network.sendToServer(new PacketScreenInteraction.PacketItemStack(screen.xCoord,screen.yCoord,screen.zCoord, x, y, z, interact, stack));
						/*
