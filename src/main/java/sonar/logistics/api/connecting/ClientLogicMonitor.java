...
public class ClientLogicMonitor implements IUUIDIdentity, INBTSyncable {
	...
	public ClientLogicMonitor() {}

	public ClientLogicMonitor(ILogicMonitor emitter) {
		this.uuid.setObject(emitter.getIdentity());
		this.coords.setCoords(emitter.getCoords());
		this.maxInfo.setObject(emitter.getMaxInfo());
	}

	public ClientLogicMonitor(UUID uuid, BlockCoords coords, int maxInfo) {
		this.uuid.setObject(uuid);
		this.coords.setCoords(coords);
		this.maxInfo.setObject(maxInfo);
	}

	public ClientLogicMonitor copy() {
		return new ClientLogicMonitor(uuid.getUUID(), coords.getCoords(), maxInfo.getObject());
	}
