...
public class PracticalLogisticsAPI {
	...
	public static void registerInfoProvider(TileProvider provider) {
		if (provider != null) {
			try {
				Class recipeClass = Class.forName("sonar.logistics.info.providers.InfoProviderRegistry");
				Method method = recipeClass.getMethod("addProvider", TileProvider.class);
				method.invoke(null, provider);
			} catch (Exception exception) {
				System.err.println("Practical Logistics API : FAILED TO REGISTER INFO PROVIDER" + exception.getMessage());
			}
